...
export const QUIZ = [
  ...
  },
  {
    ...
    explain: '付息 = 利率 × 国债余额，0.5% × 41.23 万亿 ≈ 2,062 亿。赤字率锚定时中央支出总额不变，付息多出多少，"其它"就少多少——这就是利息的挤出效应。',
    tab: 'b26',
  },
  {
    id: 'land-general',
  ...
  },
  {
    ...
    target: 'p_d_2035',
    choices: [
      { text: () => '不变：赤字率不变，债务每年增加同样多，利息只是挤占其他支出', value: () => 0 },
      { text: () => '上升约 5 个百分点', value: () => 0.05 },
      { text: () => '上升约 10 个百分点', value: () => 0.1 },
      { text: () => '下降约 3 个百分点', value: () => -0.03 },
    ],
    explain: '推演规则为"赤字率不变"时，每年新增债务 = 赤字率 × GDP，与利率无关。利率上升只改变赤字的构成：付息多了，非付息支出少了。切换到"支出增速不变"规则，利率才会推高负债率。',
    tab: 'proj',
    fmt: PP,
  },
  {
    id: 'rebate-local',
    answer: 0, // 出题人预期的答案，测试会核对模型算出来的是否一致
    q: '2025 年出口退税多退 1,000 亿元。2026 年地方一般公共预算收入会怎样？',
    setup: { changes: [{ id: 'rebate', add: 1000 }] },
    target: 'rl26',
    choices: [
      { text: () => '不变：出口退税全部由中央负担', value: () => 0 },
      { text: (b) => `约 −${Math.round(500 * (1 + b.g_rl))} 亿：退税按增值税五五分担`, value: (b) => -500 * (1 + b.g_rl) },
      { text: (b) => `约 −${Math.round(1000 * (1 + b.g_rl))} 亿：退的是地方的税`, value: (b) => -1000 * (1 + b.g_rl) },
      { text: () => '约 +500 亿', value: () => 500 },
    ],
    explain: '2015 年起出口退税（增值税部分）全部由中央财政负担，地方按基数定额上解。所以多退的 1,000 亿全部减少中央收入，地方收入基数不变。',
    tab: 'y25',
  },
  {
    id: 'transfer-national',
    answer: 0, // 出题人预期的答案，测试会核对模型算出来的是否一致
    q: '中央对地方转移支付多给 1,000 亿（赤字率锚定、本级"其它"吸收、地方以收定支）。2026 年全国一般公共预算支出会怎样？',
    setup: { modes: { c26: 'rate', absorb: 'other', l26: 'deficit' }, changes: [{ id: 'g_tr', add: 1000 / (104150 / 1.022) }] },
    target: 'e26',
    choices: [
      { text: () => '不变：中央本级少花 1,000，地方多花 1,000', value: () => 0 },
      { text: () => '增加 1,000 亿', value: () => 1000 },
      { text: () => '增加 2,000 亿：中央、地方各算一次', value: () => 2000 },
      { text: () => '减少 1,000 亿', value: () => -1000 },
    ],
    explain: '转移支付是中央和地方之间的内部往来。全国支出 = 中央本级 + 预备费 + 地方支出，转移支付不单独计入。赤字率锚定时中央支出总额不变，多转给地方的钱来自本级"其它"，全国支出总额不变，只是结构从中央移到了地方。',
    tab: 'b26',
  },
  {
    id: 'interaction',
    answer: 1, // 出题人预期的答案，测试会核对模型算出来的是否一致
    q: '赤字率从 4% 提到 5%，同时 2026 年名义增速从 5% 提到 7%（赤字率锚定、本级"其它"吸收）。中央本级"其它"支出增加多少？',
    setup: { modes: { c26: 'rate', absorb: 'other' }, changes: [{ id: 'dr26', set: 0.05 }, { id: 'g_nom', set: 0.07 }] },
    target: 'oth26',
    choices: [
      { text: (b) => `约 ${N(0.01 * b.gdp26 + 0.02 * (b.rc25 * b.eps + b.dr26 * b.gdp25))} 亿：两项单独的效果直接相加`, value: (b) => 0.01 * b.gdp26 + 0.02 * (b.rc25 * b.eps + b.dr26 * b.gdp25) },
      { text: (b) => `约 ${N(0.01 * b.gdp26 + 0.02 * (b.rc25 * b.eps + b.dr26 * b.gdp25) + 0.01 * 0.02 * b.gdp25)} 亿：两项之和，再加一个交互项`, value: (b) => 0.01 * b.gdp26 + 0.02 * (b.rc25 * b.eps + b.dr26 * b.gdp25) + 0.01 * 0.02 * b.gdp25 },
      { text: (b) => `约 ${N(0.01 * b.gdp26)} 亿：只有赤字率起作用`, value: (b) => 0.01 * b.gdp26 },
      { text: (b) => `约 ${N(0.02 * (b.rc25 * b.eps + b.dr26 * b.gdp25))} 亿：只有增速起作用`, value: (b) => 0.02 * (b.rc25 * b.eps + b.dr26 * b.gdp25) },
    ],
    explain: '赤字 = 赤字率 × GDP₂₀₂₅ × (1 + 名义增速)，两个旋钮相乘。展开：Δ赤字 = Δ赤字率 × GDP₂₀₂₆ + 赤字率 × GDP₂₀₂₅ × Δ增速 + Δ赤字率 × GDP₂₀₂₅ × Δ增速。最后一项就是交互项 = 1% × 2% × 140.24 万亿 ≈ 280 亿：多借的那 1 个百分点，是按更大的 GDP 算的。收入随增速多出的部分（中央收入 × 弹性 × 2%）没有交互项。到「影响与敏感度 → 双参数相图」可以看到等值线因此弯曲。',
    tab: 'sens',
    phase: 'oth',
  },
];
