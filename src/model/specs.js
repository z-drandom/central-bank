...
export function buildSpecs(modes = DEFAULT_MODES) {
  ...
  // ---------------- 十年推演 ----------------
  {
    ...
    f('hib_ratio', 'η', '隐性债务中计息部分的比例', 'hib_sel / max(hsel, 1)', { unit: 'pct', note: '宽口径中的"其它负债"不计息；官方口径和有息口径下为 100%。分母里 max{…, 1} 的 1 是防止除以零的保护值，单位是亿元（其余两项显示为万亿元），隐性债务不为 0 时不起作用。' });
    f(`p_Iall_${y0}`, `I^{all}_{${y0}}`, `${y0}年全口径付息`, 'int_all26', { note: '一般预算付息 + 专项债付息（政府性基金预算）+ 城投计息债务付息。' });
    f(`p_iball_${y0}`, `i^{all}_{${y0}}`, `${y0}年全口径付息/一般预算收入`, `p_Iall_${y0} / p_R_${y0}`, { unit: 'pct' });
    ...
    for (let y = y0 + 1; y <= PROJ_END; y++) {
      ...
      f(`p_Iall_${y}`, `I^{all}_{${y}}`, `${y}年全口径付息`, `p_I_${y} + p_rl_${y} * p_Bls_${p} + rh * p_H_${p} * hib_ratio`, {
        note: '全口径付息 = 一般预算付息 + 专项债付息 + 城投计息债务付息。置换把 5% 左右的城投债换成地方债，这一项会下降。',
      });
      f(`p_iball_${y}`, `i^{all}_{${y}}`, `${y}年全口径付息/一般预算收入`, `p_Iall_${y} / p_R_${y}`, { unit: 'pct' });
      if (M.proj === 'rate') {
        f(`p_D_${y}`, `D_{${y}}`, `${y}年赤字`, `pdr * p_Y_${y}`, {});
        f(`p_E_${y}`, `E_{${y}}`, `${y}年一般预算支出`, `p_R_${y} + p_D_${y} + p_T_${y}`, {});
        f(`p_PE_${y}`, `PE_{${y}}`, `${y}年非付息支出`, `p_E_${y} - p_I_${y}`, { note: '推演规则为"赤字率不变"时，付息越多，能花在其他地方的钱越少。' });
      } else {
