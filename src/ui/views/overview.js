...
export default function overview(app) {
  ...
  const tour = h('div', { class: 'sheet tour', hidden: !!load(TOUR, false) },
    ...
    h('div', { class: 'tour-steps' },
      ...
      h('div', {}, h('b', {}, '② 拧旋钮'), h('p', {}, '参数面板（宽屏在右侧，手机上点"调参数"）里每个滑杆的金色刻度是原图数值。拖动后，页面底部的"传导链"按顺序列出每个被牵动的数字。')),
  ...
  const el = h('div', { style: { display: 'contents' } },
    tour,
    h('div', { class: 'sheet' },
      h('h3', {}, '四张图是一台机器', h('small', {}, '方框 = 一张图；箭头上写着连接它们的公式。拖动参数面板里的旋钮，看变化沿哪几条箭头传播（亮起的箭头）')),
      map,
