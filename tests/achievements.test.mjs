...
test('成就：闪电手只认 30 秒以内（含）完成的任务', () => {
  const s = {};
  recordEvent(s, 'mission', { stars: 2, streak: 1, secs: 31 });
  recordEvent(s, 'mission', { stars: 2, streak: 2 });
  assert.ok(!checkAchievements([], s, null).includes('lightning'));
  recordEvent(s, 'mission', { stars: 2, streak: 3, secs: 30 });
  assert.ok(checkAchievements([], s, null).includes('lightning'));
});
