...
await t('历史：列出走过的状态，点一下回到那里，且可撤销', async () => {
  const page = await newPage();
  await page.goto(URL + '#b26', { waitUntil: 'domcontentloaded' });
  await page.evaluate(() => { __fiscal.setMany({ t_vat: 60000 }); __fiscal.setMany({ dr26: 0.05 }); });
  await page.waitForTimeout(150);
  await page.locator('button[aria-label="历史"]').click();
  const items = page.locator('.finder.hist .finder-item');
  assert.equal(await items.count(), 3, '当前 + 2 个历史状态');
  assert.match(await items.nth(1).innerText(), /国内增值税 60,000/);
  await items.nth(2).click();
  await page.waitForTimeout(200);
  assert.equal(await page.evaluate(() => __fiscal.sim.changedIds().length), 0, '回到原图');
  await page.locator('button[aria-label="撤销"]').click();
  await page.waitForTimeout(150);
  assert.equal(await page.evaluate(() => __fiscal.v.dr26), 0.05, '撤销回到跳转前');
  assert.deepEqual(page.errors, []);
  await page.close();
});
